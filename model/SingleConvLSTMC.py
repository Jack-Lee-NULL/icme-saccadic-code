...
class SingleConvLSTMC(SingleConvLSTMA):
    ...
    def _init_holder(self):
        ...
        self._inputs_2 = tf.placeholder(tf.float32,
                (None, self._shape[2], self._shape[3], self._inputs_channel[0]))
        self._inputs = (self._inputs_1, self._inputs_2)
    ...
    def _attention_net(self, inputs, weight, stage='1'):
        with tf.variable_scope('attention_net_'+stage, reuse=tf.AUTO_REUSE):
            filters1 = tf.get_variable('filter1',
                    ...
                    dtype=tf.float32, initializer=tf.initializers.truncated_normal(1.0))
            ...
            inputs_data = tf.nn.relu(inputs_data)
    ...
    def _generate_saliency(self, feature, mode='train', scope='s'):
        with tf.variable_scope('GEN_Saliency'+scope, reuse=tf.AUTO_REUSE):
            saliency = tf.concat(feature, axis=3)
            filters1 = tf.get_variable('filters1', shape=[3, 3, saliency.shape.as_list()[3], 1], dtype=tf.float32)
            bias1 = tf.get_variable('bias1', shape=[1], dtype=tf.float32)
            saliency = tf.nn.conv2d(saliency, filters1,
                    strides=[1, 1, 1, 1], padding='SAME') + bias1
            if mode == 'test':
                saliency = tf.sigmoid(saliency)
            return saliency
    ...
    def __call__(self, mode='train'):
        ...
        if mode == 'train' or mode == 'validation':
            saliency = self._generate_saliency(saliency, mode=mode)
            return  preds, c_all, h_all, saliency
        else:
            return  preds, c_all, h_all
